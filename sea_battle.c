...
void drawShipCounts() {
    ...
    bool botProcessed[GRID_SIZE][GRID_SIZE] = { false };
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            if (botGrid[i][j].is_ship && botGrid[i][j].is_miss && !botProcessed[i][j]) {
                // Нашли подбитую палубу - проверяем весь корабль
                bool visited[GRID_SIZE][GRID_SIZE] = { false };
                int shipSize = 0;
                bool fullyDestroyed = true;

                // Нерекурсивный обход корабля
    ...
    // Аналогичный подсчет для игрока
