...
static bool should_disable_logging() {
    char response[8] = {0};
    printf("Turn off the logging? (y/N): ");
...
int main(int argc, char* argv[]) {
    ...
    bool logging_disabled = should_disable_logging();

    char log_filename[256];
    bool logging_enabled = !logging_disabled;
    if (logging_enabled) {
        ...
        printf("Logging has been turned off by the user. Running without logging...\n");
    }
    ...
    if (logging_enabled) {
        ...
        printf("Logging has been turned off.\n");
    }
    printf("---------------\n");
